    ...
    this.getLinkColor = function (d, minColor, maxColor) {
        if(d.data && d.data.length > 0 && d.data[0].get && d.data[0].get('isEdge')) {
            ...
            if(window.application.state.colorFilterActive && d.source.color && d.source.color == d.target.color) {
                d.color = d.source.color.indexOf('#') >= 0 ? colors.getLighterColorHex(d.source.color) : d.source.color;
                if(d.color.indexOf('#') >= 0) {
                    // limit the brightness
                    var rgb = colors.getRgbaFromHex(d.color);
                    var brightness = (rgb[0] * 2 + rgb[1] + rgb[2] * 3) / 6;
                    if(brightness > 200)
                        d.color = colors.getDarkerColorHex(d.source.color,.8);
                }
            }
            ...
            d.color = colors.rgba(colors.colorBlend(colors.getRgbaFromHex(minColor || graph.stylelib().colors.linkMin), colors.getRgbaFromHex(maxColor || graph.stylelib().colors.linkMax), d.ratio));
        return d.color;
    };
